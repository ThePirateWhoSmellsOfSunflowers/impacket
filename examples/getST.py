...
from pyasn1.type.univ import noValue, BitString

from impacket import version
from impacket.examples import logger
from impacket.examples.utils import parse_identity
from impacket.krb5 import constants, types, crypto, ccache
from impacket.krb5.asn1 import AP_REQ, AS_REP, TGS_REQ, Authenticator, TGS_REP, seq_set, seq_set_iter,PA_S4U_X509_USER, PA_FOR_USER_ENC, S4UUserID, \
    Ticket as TicketAsn1, EncTGSRepPart, PA_PAC_OPTIONS, EncTicketPart
from impacket.krb5.ccache import CCache, Credential
from impacket.krb5.crypto import Key, _enctype_table, _HMACMD5, _AES256CTS, _SHA1AES128, _SHA1AES256, Enctype, string_to_key
from impacket.krb5.constants import TicketFlags, encodeFlags
from impacket.krb5.kerberosv5 import getKerberosTGS, getKerberosTGT, getDMSA, sendReceive
...
class GETST:
    ...
    def doDMSA(self, tgt, cipher, oldSessionKey, sessionKey, nthash, aesKey, kdcHost):
        decodedTGT = decoder.decode(tgt, asn1Spec=AS_REP())[0]
        # Extract the ticket from the TGT
        ticket = Ticket()
        ticket.from_asn1(decodedTGT['ticket'])
        ...
        princip = Principal(self.__options.impersonate, type=constants.PrincipalNameType.NT_PRINCIPAL.value)
        seq_set(paS4Ux509['user-id'], 'cname', princip.components_to_asn1)
        paS4Ux509['user-id']['crealm'] = domain.upper()
        paS4Ux509['user-id']['subject-certificate'] = noValue
        paS4Ux509['user-id']['options'] = 0x28000000

        print(encoder.encode(paS4Ux509['user-id']['options']))
        ...
        if logging.getLogger().level == logging.DEBUG:
            logging.debug('S4UUserID')
            print(decoder.decode(encoded)[0])
        ...
        checkSum = _SHA1AES128.checksum(sessionKey, 26, encoded)
        print(encoded)
        print(checkSum)
        print('OH')
        ...
        paS4Ux509['checksum'] = noValue
        paS4Ux509['checksum']['cksumtype'] = 15
        paS4Ux509['checksum']['checksum'] = checkSum

        if logging.getLogger().level == logging.DEBUG:
            logging.debug('PA_S4U_X509_USER')
            print(paS4Ux509.prettyPrint())

        encodedpaS4Ux509 = encoder.encode(paS4Ux509).replace(b'\x03\x05\x02\xa0',b'\x03\x05\x00\x28')
        print(encodedpaS4Ux509)
        tgsReq['padata'][1] = noValue
        tgsReq['padata'][1]['padata-type'] = int(constants.PreAuthenticationDataTypes.PA_X509_USER.value)
        tgsReq['padata'][1]['padata-value'] = encodedpaS4Ux509

        paPacOptions = PA_PAC_OPTIONS()
        paPacOptions['flags'] = constants.encodeFlags((constants.PAPacOptions.branch_aware.value,))
    ...
    def run(self):
        ...
        if self.__options.impersonate is None:
            ...
            tgs, cipher, oldSessionKey, sessionKey = getDMSA(serverName, domain, self.__kdcHost, tgt, cipher, sessionKey, self.__options.renew)
            ...
            try:
                ...
                if self.__additional_ticket is not None:
                    ...
                    tgs, cipher, oldSessionKey, sessionKey = self.doDMSA(tgt, cipher, oldSessionKey, sessionKey, unhexlify(self.__nthash), self.__aesKey, self.__kdcHost)
